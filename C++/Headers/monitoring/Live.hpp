...
*/
// ===================================================================================
// Live.hpp: A cross-platform system monitoring header-only library for C++.
//
// This library provides classes to monitor CPU, RAM, Disk, Network, and GPU
// performance on both Windows and Linux systems.
// ===================================================================================
#ifndef LIVE_HPP
#define LIVE_HPP
#pragma once
...
#include "../../../include/cpuidHelpers.hpp"
#ifdef _WIN32
 ...
 */
inline double MonitorCpuRealtime_Nt(int intervalMs) {
  SYSTEM_INFO sysInfo;
...
inline long long get_idle_time(const CpuTimes &t) { return t.idle + t.iowait; }
inline long long get_total_time(const CpuTimes &t) {
...
inline std::map<std::string, DiskStats> get_disk_stats() {
...
inline std::map<std::string, NetworkStats> get_network_stats() {
...
inline double
GetSensorTempByKeywords(const std::vector<std::string> &keywords,
                        const std::vector<std::string> &MatchKeyWords = {}) {
  double temp = -1.0;
  ...
  while ((chip = sensors_get_detected_chips(nullptr, &chip_nr)) != nullptr) {
    ...
    while ((feature = sensors_get_features(chip, &feature_nr)) != nullptr) {
      ...
      if (match) {
        ...
        int sub_nr = 0;
        while ((sub = sensors_get_all_subfeatures(chip, feature, &sub_nr)) !=
               nullptr) {
...
inline double GetCpuPackageTemp() {
  return GetSensorTempByKeywords({"Package id", "Physical id", "Tdie"});
}

inline double GetMotherboardTemp() {
  return GetSensorTempByKeywords({"MB ", "SYSTIN", "System", "Board"},
                                 {"temp1"});
}

inline double GetVRMTemp() { return GetSensorTempByKeywords({"VRM"}); }

inline double GetChipsetTemp() {
  return GetSensorTempByKeywords({"PCH", "Chipset"});
}

inline double GetDIMMTemp() {
  return GetSensorTempByKeywords({"DIMM", "Memory"});
}

inline double GetDriveTemp() {
  return GetSensorTempByKeywords({"drive", "HDD", "SSD", "nvme"});
}
...
public:
   ...
   */
  inline double get_usage(int interval_ms) const {
#ifdef _WIN32
   ...
   * @return A map of time counters or an integer with the core count.
   */
  inline std::variant<int, std::map<std::string, double>>
  CpuSnapShot(int core, bool coreNumbers = false, bool Kernel = true,
              bool User = true, bool Idle = true, bool PureKernalTime = false) {
    if (core < 0) {
...
public:
   ...
   */
  inline std::variant<double, std::vector<double>>
  get_usage(bool Raw = false) const {
#ifdef _WIN32
  ...
  struct Phys {
    double Free; //Free Physical RAM
    double Used; // Used Physical RAM
  };

  struct Proc {
    int TotalProc; // Total Processes
    std::vector<std::pair<std::string, double>> info; // Process Name,Usage
  };

  struct Total {
    double TotalPhys; // Total Physical RAM Percentage
    bool status; // status
  };
  ...
  RamInfo getRamInfo() {
    ...
    double totalPhys = static_cast<double>(memInfo.ullTotalPhys);
    double freePhys = static_cast<double>(memInfo.ullAvailPhys);
    double usedPhys = totalPhys - freePhys;

    info.physRam.Free = freePhys;
    info.physRam.Used = usedPhys;
    ...
    info.procRam.TotalProc = static_cast<int>(cProcesses);
    ...
    for (unsigned int i = 0; i < cProcesses; i++) {
      ...
      if (hProcess) {
        ...
        if (GetProcessMemoryInfo(hProcess, (PROCESS_MEMORY_COUNTERS *)&pmc, sizeof(pmc))) {
          double privateUsage = static_cast<double>(pmc.PrivateUsage);
          ...
          info.procRam.info.emplace_back(processName, privateUsage);
        }
        CloseHandle(hProcess);
      }
    }

    double physPercent = (usedPhys / totalPhys) * 100.0;
    info.totalTRam.TotalPhys = physPercent;
    info.totalTRam.status = (physPercent > 65.0);
...
public:
  /**
   * @brief Constructs a LiveDisk monitor.
   * @param mode 0 for '% Disk Time' (Windows-only), 1 for R/W speed in MB/s.
   */
  inline LiveDisk(int mode) : mode(mode) {
#ifdef _WIN32
    ...
    if (mode == 0) {
      ...
      throw std::invalid_argument("Invalid mode for LiveDisk. Use 0 for "
    ...
    if (mode != 1)
      throw std::invalid_argument(
          "Invalid mode for LiveDisk on Linux. Only Mode 1 is supported.");
#endif
  }
   ...
   * write_MBps}.
   */
  inline std::variant<double, std::vector<std::pair<std::string, double>>>
   ...
   * @return True ifusage is high, false otherwise.
   */
  inline bool HighDiskUsage(double threshold_mbps = 80.0) {
    if (mode != 1) {
...
/**
 * @class LiveProc
 * @brief Provides process monitoring functionalities.
 */
#ifdef _WIN32
class LiveProc {
  

  double GetRamByPID(DWORD pid) {
    HANDLE hp =
        OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, pid);
    if (!hp) {
      return -1.00;
    }
    PROCESS_MEMORY_COUNTERS pmu;
    if (!GetProcessMemoryInfo(hp, &pmu, sizeof(pmu))) {
      CloseHandle(hp);
      return -1.00;
    }
    CloseHandle(hp);
    return (double)pmu.WorkingSetSize;
  }

  ULONGLONG FileTime(const FILETIME &ft) {
    return ((ULONGLONG)ft.dwHighDateTime << 32) | ft.dwLowDateTime;
  }

  std::vector<double> GetTimesByPID(DWORD pid) {
    ...
    CloseHandle(hp);
    double kt = FileTime(ftKernel) / 1e7;
    double ut = FileTime(ftUser) / 1e7;
    return {kt, ut};
  }

  std::vector<std::pair<std::string, std::string>> GetDllsByPID(DWORD pid) {
    std::vector<std::pair<std::string, std::string>> dlls;
    HANDLE hp = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE | TH32CS_SNAPMODULE32, pid);
    if (hp == INVALID_HANDLE_VALUE) {
      return dlls;
    }
    MODULEENTRY32 me32;
    me32.dwSize = sizeof(me32);
    if (Module32First(hp, &me32)) {
      do {
        std::string modName = me32.szModule;
        std::string modPath = me32.szExePath;

        dlls.emplace_back(modName, modPath);
      } while (Module32Next(hp, &me32));
    }
    CloseHandle(hp);
    return dlls;
  }
...
public:
  void GetProcs(bool Dlls = false) {
    TotalRAM = 0; // Reset the total RAM
    Procs.clear(); // Clear the vector
    ...
    do {
      ...
      p.Priority = std::to_string(pe.pcPriClassBase);
      p.Ram = GetRamByPID(pe.th32ProcessID);
      TotalRAM = TotalRAM + p.Ram;
      p.Times = GetTimesByPID(pe.th32ProcessID);
      if(Dlls){p.Dlls = GetDllsByPID(pe.th32ProcessID);}
      Procs.push_back(p);
  ...
  LiveProc(bool WithDlls = false) { this->GetProcs(WithDlls); }

  void Update(bool WithDlls = false) { this->GetProcs(WithDlls); }

  std::vector<Proc> GetProcsP() { return Procs; }

  double GetTotalRAM() { return TotalRAM; }
  ...
  bool KillAllProcsWithSameName(std::string name, std::vector<int> &error) {
    for (auto &p : Procs) {
      if (p.name == name) {
        if (!KillProc(p.pid)) {
          error.push_back(p.pid);
        }
      }
    }
    return true;
  ...
  double GetTotalRAMByName(std::string name) {
    double total = 0;
...
public:
   ...
   */
  inline double get_usage(int interval_ms = 1000) const {
   ...
   */
  inline double get_average_usage(int interval_ms = 1000) const {
    if (!query || counters.empty()) {
      throw std::runtime_error("GPU Monitor was not initialized correctly.");
    }
   ...
   */
  inline double get_max_usage(int interval_ms = 1000) const {
    if (!query || counters.empty()) {
      throw std::runtime_error("GPU Monitor was not initialized correctly.");
    }
   ...
   */
  inline size_t get_counter_count() const { return counters.size(); }
};
...
public:
  ...
  inline double get_temp() const {
    return temp; // -1 = error
  }
  inline double get_max_temp() const { return max_temp; }
  inline double get_avg_temp() const { return avg_temp; }
  inline double get_fan_rpm() const { return fan_rpm; }
  inline void update() {
...
public:
  inline TempGpu(bool init = true) {
    ...
    fan_rpm = GetGpuFanRpmTemp();   // Get the current GPU fan speed
  }
  inline double get_temp() const { return temp; }
  inline double get_fan_rpm() const { return fan_rpm; }
  inline void update() {
    UpdateHardwareMonitorTemp();
    temp = GetGpuTemperatureTemp(); // Get the current GPU temperature
...
public:
  inline TempOther(bool init = true) {
    if (init) {
      if (InitHardwareTempMonitor() != 0) {
        throw std::runtime_error(
            "LiveView: Failed to initialize hardware temperature monitor Check "
  ...
  inline double get_mb_temp() const { return mb_temp; }
  inline double get_Storage_temp() const { return storage_temp; }
  inline void update() {
    UpdateHardwareMonitorTemp();
    mb_temp = GetMotherboardTemperatureTemp();
    storage_temp = GetStorageTemperatureTemp();
...
/**
 *@class ManageTemp
 *@brief Class to manage temperature sensors for Windows
 */
class ManageTemp {
public:
  inline void Init() {
  ...
  inline void Close() { ShutdownHardwareTempMonitor(); }
  inline void Update() { UpdateHardwareMonitorTemp(); }
};

class Sensor {
  // Helper Function
  inline std::vector<std::string> ConvertC(char **sensors, int count) {
  ...
  inline std::vector<double> ConvertD(double *sensors, int count) {
...
public:
  inline void GetData(bool init = false) {
    if (init) {
      if (InitHardwareTempMonitor() != 0) {
        throw std::runtime_error(
            "LiveView: Failed to initialize hardware temperature monitor Check "
  ...
  inline Sensor(bool init = true) { GetData(init); }

  inline double GetValueByName(std::string name) {
    bool found = false;
  ...
  inline auto getAllSensors() { return sensor_names; }
  inline auto getAllFanRPMs() { return fan_rpms; }
  inline void update() {
    UpdateHardwareMonitorTemp();
    GetData();
  }
  inline void ReGet() { GetData(); }
};
...
public:
   ...
   */
  inline static std::vector<BYTE> RSMB() {
...
public:
  ...
  inline double getCpuTemp() const { return cputemp; }
  inline double getChipsetTemp() const { return chipstemp; }
  inline double getMotherboardTemp() const { return mbtemp; }
  inline double getVRMTemp() const { return memorytemp; }
  inline double getDriveTemp() const { return storagetemp; }
  inline std::vector<std::string> getAllSensorNames() const {
  ...
  findSensorName(std::string name) const {
    ...
    for (size_t i = 0; i < sensors_names.size(); i++) {
  ...
  inline double GetSensorTemp(std::string name, bool Match) const {
    if (name.empty()) {
      return -1;
    }
  ...
  inline void update(bool names = false) {
    if (names)
      sensors_names = GetAllSensorNames();
    cputemp = GetCpuPackageTemp();
    chipstemp = GetChipsetTemp();
