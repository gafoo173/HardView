...
class build_ext(_build_ext):
    ...
    2. Copy necessary DLLs for the LiveView extension on 64-bit Windows.
    """
    def run(self):
        # Run the original build process first
        super().run()

        # NEW: After building, check if we are on 64-bit Windows and copy DLLs
        if sys.platform.startswith('win') and sys.maxsize > 2**32:
            ...
            # The custom get_ext_filename gives us the path within the build directory.
            ...
            # Define the source path for the DLLs
            dll_source_path = 'src/LiveView/64/*.dll'
